...
function drawLives() {
  ...
  ctx.font = "20px Arial";
  ctx.textAlign = "left";
  ctx.fillText(`Vite: ${hearts}`, 10, 60);
}
...
function drawGameOverScreen() {
  ...
  ctx.fillStyle = "#fff";
  ctx.font = "20px Arial";
  ctx.fillText("Rigioca", canvas.width / 2, btnY + 27);
}
...
// Flash rosso breve quando prendi danno
let flashTimer = 0;
...
document.addEventListener("keydown", (e) => {
  if (e.code === "Space" && !gameOver) {
    isPaused = !isPaused;
    if (!isPaused) requestAnimationFrame(loop); // Riprendi solo se era in pausa
  }
});
...
function drawPauseScreen() {
  ...
  ctx.font = "20px Arial";
  ctx.fillText("Premi SPAZIO per continuare", canvas.width / 2, canvas.height / 2 + 20);
}
...
const ecoTips = [
  "Ricicla sempre la plastica per aiutare il pianeta!",
  "Evita prodotti monouso, scegli alternative riutilizzabili.",
  "Partecipa a eventi di pulizia locali per il mare e i fiumi.",
  "Riduci l'uso di plastica usa e getta.",
  "Supporta le politiche per la riduzione della plastica.",
  "Usa borracce invece di bottiglie di plastica.",
];

function updateDifficulty() {
  if (score < 10) return "Facile";
  else if (score < 30) return "Media";
...
setInterval(updateEcoTip, 15000); // cambia ogni 15s

// Modifica la funzione movePlastics per aumentare avoidedCount
function movePlastics() {
  plastics.forEach(p => {
    p.y += p.speed;
  });
  // Rimuovi plastiche fuori dallo schermo e aumenta punteggio o avoidedCount
  for (let i = plastics.length - 1; i >= 0; i--) {
    if (plastics[i].y > canvas.height) {
      plastics.splice(i, 1);
      if (!gameOver) {
...
function loop() {
  ...
  // Se è attivo il flash rosso, disegna rettangolo rosso trasparente sopra
  if (flashTimer > 0) {
